    ...
    const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formState) {
            ...
            if (error) {
                ...
                console.log(data + 'se ha logueado ')
            ...
            if (error) {
                ...
                Swal.fire({
                    ...
                    text: 'Para completar su registro, verifica tu correo electrónico.',
                })
            }
            ...
            <section className="min-h-screen flex items-stretch text-white ">
                <div className="lg:flex w-1/2 hidden bg-gray-500 bg-no-repeat bg-cover relative items-center bg-[url(https://images.unsplash.com/photo-1577495508048-b635879837f1?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=675&q=80)]">
                    ...
                    <div className="w-full px-24 z-10">
                        ...
                        <p className="text-3xl my-4">Capture your personal task in unique way, anywhere.</p>
                    </div>
                        ...
                        <form onSubmit={handleLogin} className="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
                            <div className="flex gap-4">
                                ...
                                <button
                                    ...
                                    className={` cursor-pointer relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-300 focus:outline-none ${formState ? 'bg-indigo-500' : 'bg-gray-300'
                                        }`}
                                >
                                    <span
                                        className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform duration-300 ${formState ? 'translate-x-6' : 'translate-x-1'
                                            }`}
                                    />
                                </button>
                            </div>
                            <div className="pb-2 pt-4">
                                <input type="email" name="email" id="email" placeholder="Email" className="block w-full p-4 text-lg rounded-sm bg-gray-900" onChange={(e) => { setlogin({ ...login, email: e.target.value }) }} required />
                            </div>
                            <div className="pb-2 pt-4">
                                <input className="block w-full p-4 text-lg rounded-sm bg-gray-900" type="password" name="password" id="password" placeholder="Password" onChange={(e) => { setlogin({ ...login, password: e.target.value }) }} required />
                            </div>
                            <div className="text-gray-400 hover:underline hover:text-black">
                                <a href="#">Forgot your password?</a>

                            </div>
                            <div className="px-4 pb-2 pt-4 flex gap-4">
                                <button type="submit" className="uppercase block w-full p-4 text-lg rounded-full bg-indigo-500 hover:bg-indigo-600 focus:outline-none">{formState ? 'register' : 'login'}</button>
                            </div>
